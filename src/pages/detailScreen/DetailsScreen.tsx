...
import { ArrowLeft, Info, MapPin, TrendingUp } from "lucide-react";
...
export function DetailsScreen() {
	...
	const { dataPoints } = useAppSelector((state) => state.dashboard);

	const dataPoint = dataPoints.find((p) => p.id === dataPointId);
						...
						<div>
							...
							<div className="space-y-3">
								<div className="flex justify-between">
									...
									<span className="font-medium">{dataPoint.x.toFixed(2)}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600">Y Position:</span>
									<span className="font-medium">{dataPoint.y.toFixed(2)}</span>
								</div>
								<div className="flex justify-between">
